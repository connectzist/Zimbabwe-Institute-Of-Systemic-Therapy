                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach($student_results as $diploma_result)
                            <tr>
                                <td>{{ $diploma_result->module->module_name ?? 'No module assigned' }}</td>
                                <td>{{ $diploma_result->research_mark ?? 'N/A' }}</td>
                               ...
                               <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a class="dropdown-item" href="{{ route('diploma_results.diplomafinaledit', $diploma_result->id) }}">
                                                    <i class="fas fa-edit me-2"></i> Edit
                                                </a>
                                            </li>
                                            <li>
                                                <form action="{{ route('diploma_results.diplomafinaldelete', $diploma_result->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?');" class="m-0 p-0">
                                                    @csrf
                                                    @method('DELETE')
